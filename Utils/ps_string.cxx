...
char* ps::merge_strings(const char *fmt, ...)
{
		c_ptr<char>	merge_string;
		va_list		ap;

		merge_string.set_free(false);
		int bufsiz = strlen(fmt) + 512;
		merge_string.alloc(bufsiz);

		va_start(ap, fmt);
		// Make sure we have enough memory to store parameters
		int need = vsnprintf(merge_string.ptr(), bufsiz, fmt, ap) + 1;
		if (need > bufsiz)
		{
			merge_string.realloc(need);
			vsnprintf(merge_string.ptr(), need, fmt, ap);
		}
		va_end(ap);

		return merge_string.ptr();
}
