...
#ifdef _TC_10
bool ps::get_preference(const char *pref_name, int *num_prefs, char ***prefs)
{
	logical result = true;
...
bool ps::get_preference(const char *pref_name, char **pref)
{
	logical result = true;
...
bool ps::get_preference(const char *pref_name, int *pref)
{
	logical result = true;
...
bool ps::get_preference(const char *pref_name, logical *pref)
{
	logical result = true;

	try
	{
		itk(PREF_ask_logical_value(pref_name, 0, pref));
	}
	catch (tcexception& e)
	{
		if (e.getstat() == PF_NOTFOUND)
...
bool ps::get_preference(const char *pref_name, TC_preference_search_scope_t scope, logical *pref)
{
	logical result = true;
	TC_preference_search_scope_t oldScope;
