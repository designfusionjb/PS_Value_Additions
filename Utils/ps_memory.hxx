...
namespace ps
{
	...
	public:
		...
		void realloc()
		{
			...
			sm_alloc(tmpPtr, T, m_alloc_size);
			memcpy(tmpPtr, m_ptr, m_alloc_size * sizeof(T));
			dealloc();
			m_ptr = tmpPtr;
		}
		...
		c_ptr<T>& operator=(const c_ptr<T>& other)
		{
			if (this != &other)
			{
				this->m_alloc_chunk = other.m_alloc_chunk;
				this->m_alloc_size = other.m_alloc_size;
				this->m_free = other.m_free;
				this->m_size = other.m_size;
				this->alloc(other.m_alloc_size);
				memcpy(this->m_ptr, other.m_ptr, m_size * sizeof(T));
			}
			return *this;
		}
		const T& operator[](const int i) const { return m_ptr[i] };
		//! Remove an object from this array
		void remove(T anObject)
		{
			c_ptr<T>	tmpObj;
			tmpObj.set_free(false);
